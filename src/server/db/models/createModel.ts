...
> = {
    create: (data: Omit<ModelType, "id" | "createdAt" | "updatedAt">) => Promise<ModelType>
    bulkCreate: (data: Omit<ModelType, "id" | "createdAt" | "updatedAt">[]) => Promise<ModelType[]>
    ...
    findMany: (orgId: string, queryFn?: (query: Query) => Query) => Promise<ModelType[]>
    ...
    UNSAFE_findOne: (queryFn: (query: Query) => Query) => Promise<ModelType | undefined>
    search: (
...
> = {
    ...
    searchConfig: {
        handlers: Parameters<typeof makeSearchQuery<ModelType, FiltersType>>[0]["handlers"]
        select?: Parameters<typeof makeSearchQuery<ModelType, FiltersType>>[0]["select"]
...
>(
    ...
    const create = async (
        ...
        if (mutateOnCreate) {
            dataWithTimestamp = {
                ...mutateOnCreate(
                    merge.recursive(true, {}, data) as Omit<
                        ModelType,
    ...
    const updateById = async <TUpdateStrategy extends "merge" | "replace" = "merge">(
        ...
        if (mutateOnPreUpdate) {
            newData = mutateOnPreUpdate(
                oldData,
                newData as ModelType,
            ) as TUpdateStrategy extends "merge" ? Partial<ModelType> : ModelType
        }
    ...
    const updateByIdWithTx = async (
        ...
        const firestore = getFirestore()
        const updatedModel = await firestore.runTransaction(async (transaction: Transaction) => {
            ...
            const updatedDocument = merge.recursive(true, {}, docData, updateData, {
                updatedAt: new Date().toISOString(),
            }) as ModelType
    ...
    const patchById = async (id: string, fields: Partial<ModelType>): Promise<ModelType> => {
        const docRef = getCollection().doc(id)
        await docRef.update({
            ...fields,
            updatedAt: new Date().toISOString(),
    ...
    const UNSAFE_findMany = async (queryFn?: (query: Query) => Query): Promise<ModelType[]> => {
        const collection = getCollection()
    ...
    const search = async (
        ...
        const schemaDef = ModelSchema._def as {
            catchall?: unknown
