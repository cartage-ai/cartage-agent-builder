...
function captureException(level: string, messageOrError: string | Error): void {
    const parentError = messageOrError instanceof Error ? messageOrError : new Error(messageOrError)
        ...
        Sentry.withScope((scope) => {
            ...
            if (parentError instanceof XError) {
                ...
                if (parentError.sentryTags) {
                    Object.entries(parentError.sentryTags).forEach(([k, v]) => scope.setTag(k, v))
                }
