...
export class XError extends Error {
  ...
  constructor({
    ...
    this.cause = Array.isArray(cause) || cause instanceof Error || cause === null ? cause : (cause as Error)
...
export const getAggregatedMessage = (errors: Error[]): string =>
  errors
    .map((e) => e.message ?? e.name ?? JSON.stringify(e))
    .join(" -> ")
...
export const getCustomFingerPrint = (errors: Error[]): string =>
