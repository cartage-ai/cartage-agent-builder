...
export function EnvironmentsUI() {
    ...
    const deleteMutation = api.environment.delete.useMutation({
        onSuccess: () => {
            void utils.environment.list.invalidate()
        },
    })
    ...
    const errorMessage =
        error?.message ?? createMutation.error?.message ?? deleteMutation.error?.message ?? null
            ...
            <section>
                ...
                {isLoading ? (
                    ...
                    <p className="text-sm text-zinc-500">
                        No environments yet. Click “New environment” to spin up a Blaxel sandbox
                        with the Cartage Agent repo and a live preview.
                    </p>
                ) : (
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            onClick={async () => {
                                                if (
                                                    !confirm(
                                                        `Delete environment ${env.sandboxName}? This will delete both the Firestore record and the Blaxel sandbox.`,
                                                    )
                                                ) {
                                                    return
                                                }
                                                ...
                                                try {
                                                    await deleteMutation.mutateAsync({ id: env.id })
                                                } finally {
                                                    setDeletingId(null)
                                                }
                                            }}
                                            disabled={deletingId === env.id}
                                            className="rounded bg-red-100 px-2 py-0.5 text-xs text-red-700 hover:bg-red-200 disabled:opacity-50 dark:bg-red-950 dark:text-red-300 dark:hover:bg-red-900"
                                        >
